...
def apply_theme(root):
    ...
    class LandingApp:
        def __init__(self, root: tk.Tk):
            ...
            try:
                canvas.create_oval(-250, -200, 450, 500, fill=PALETTE["PRIMARY"], outline="", stipple="gray25")
                canvas.create_oval(1000, 350, 1650, 1000, fill=PALETTE["ACCENT"], outline="", stipple="gray25")
            except:
                pass

            header = ttk.Frame(canvas, style="Nav.TFrame")
            header.place(relx=0, rely=0, relwidth=1)
            ttk.Label(header, text="Diabetic Mellitus Using Foot Thermography", style="Title.TLabel").pack(anchor="w", padx=24, pady=16)
            ttk.Label(header, text="Modern UI • Theme #5", style="Sub.TLabel").pack(anchor="w", padx=24, pady=(0, 8))

            card = ttk.Frame(canvas, style="Card.TFrame", padding=32)
            card.place(relx=0.5, rely=0.5, anchor="center")
            ttk.Label(card, text="Detect early. Act confidently.", font=("Segoe UI", 20, "bold")).pack(anchor="w")
            ttk.Label(card, text="Sign in or create an account to continue", style="Muted.TLabel").pack(anchor="w", pady=(0, 20))

            actions = ttk.Frame(card, style="Card.TFrame")
            actions.pack(anchor="w")
            ttk.Button(actions, text="Login", style="Primary.TButton", command=lambda: self.launch("login.py")).pack(side="left", padx=(0, 10))
            ttk.Button(actions, text="Register", command=lambda: self.launch("register.py")).pack(side="left")

            footer = ttk.Frame(canvas, style="Nav.TFrame")
            footer.place(relx=0, rely=1, anchor="sw", relwidth=1)
            ttk.Button(footer, text="Exit", command=self.root.destroy).pack(side="right", padx=16, pady=12)
